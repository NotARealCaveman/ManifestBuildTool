...
MFvec3 Manifest_Math::Slerp(const MFvec3& v, const MFvec3& a, const MFfloat& t)
{
	...
	auto&& vDota = Dot(v, a);
	auto&& aRejv = a * vDota;
	auto&& perpav = Cross(a, v);
	auto&& aRejv1c = aRejv * (1 - c);
	auto&& perpavs = perpav * s;
	auto&& aRejv1cperpavs = aRejv1c + perpavs;
	auto&& aRejv1cperpavsvc = vc + aRejv1cperpavs;

	std::cout << aRejv1cperpavsvc << std::endl;
